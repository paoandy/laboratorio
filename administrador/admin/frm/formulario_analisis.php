        ...
        <div id="formularioSeccion">
            <?php
                ...
                $obj = $form->add('text', 'nombreseccion', '', array('placeholder' => 'Seccion de Analisis'));
                   ...
                   $obj = $form->add('hidden', 'idseccion', $query->siguiente('idseccion','seccion') );
			...
			$consulta = $query->getRowsArray('NOMBRESECCION' ,'seccion',"WHERE nombreseccion= '".$nombreseccion."'");
			 
				if ( sizeof($consulta)) { 
				 echo "<script> $(document).ready(function() { Messenger().post({ message: 'La seccion ya existe.',  showCloseButton: true }); }); </script>";	// Avisa que existe el usuario
				  $form->reset();					// Resetea el formulario
				}
				 else {
				    $query->dbInsert(array('idseccion'=>$idseccion,'nombreseccion'=>$nombreseccion),'seccion');
					    echo "<script> $(document).ready(function() { Messenger().post({ message: 'Nuevo Registro...<br><br>Seccion registrada OK',  showCloseButton: true }); }); </script>";
				    }
				    $form->reset(); 
